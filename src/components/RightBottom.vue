...
<!--      <a-->
<!--        v-if="downloadUrl"-->
<!--        class="common-right-bottom-link"-->
<!--        :href="downloadUrl"-->
<!--        target="_blank"-->
<!--      >-->
...
<!--      </a>-->
<!--      <div-->
<!--        v-else-if="updateVersion && updateBottomShow && $Electron"-->
<!--        class="common-right-bottom-link"-->
    ...
    >
      <div slot="header">
        <div class="notification-head">
          ...
          <Tag color="volcano"
            >v{{ systemVersion }} -&gt; v{{ updateVersion }}</Tag
          >
        </div>
        <div v-if="$Platform === 'mac'" class="notification-tip">
      ...
      <div slot="footer" class="adaption">
        <Button type="default" @click="updateShow = false">{{
          $L("稍后")
        }}</Button>
        <Button
          type="primary"
          :loading="updateIng"
          @click="updateQuitAndInstall"
          >{{ $L($Platform === "mac" ? "重新启动" : "立即升级") }}</Button
        >
      </div>
    </Modal>
  </div>
  ...
  mounted() {
    ...
    if (this.$Electron) {
      ...
      this.$Electron.registerMsgListener("updateDownloaded", (info) => {
        ...
        this.updateShow = !$A.strExists(
          this.updateNote,
          `[${this.updateVersion}-Silence]`
        );
      });
    }
  },
  ...
  computed: {
    ...
    showDown() {
      return (
        !this.$isEEUiApp &&
        !this.windowTouch &&
        ["login", "index", "manage-dashboard"].includes(this.$route.name)
      );
    },
      ...
      switch (publish.provider) {
        ...
        case "github":
          (async (_) => {
            ...
            if (
              cache.time &&
              cache.time + timeout > Math.round(new Date().getTime() / 1000)
            ) {
            ...
            axios
              .get(
                `https://api.github.com/repos/${publish.owner}/${publish.repo}/releases`
              )
              .then(({ status, data }) => {
                this.loadIng--;
