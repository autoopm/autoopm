...
(function (window, $, undefined) {
  ...
  localforage.config({ name: "Autoo", storeName: "common" });
  ...
  $.extend({
    ...
    TimeM(v = undefined, cm = true) {
      ...
      if (typeof v === "string" && this.strExists(v, "-")) {
        ...
        if (cm && v.indexOf(".") === -1) {
          time = parseInt(
            `${Math.round(time / 1000)}${new Date().getMilliseconds()}`
          );
        }
      } else {
        time = new Date().getTime();
     ...
     */
    isEmail(email) {
      return /^([0-9a-zA-Z]([-.\w]*[0-9a-zA-Z])*@([0-9a-zA-Z][-\w]*\.)+[a-zA-Z]*)$/i.test(
        email
      );
    },
    ...
    screenOrientation() {
      ...
      return $A(window).width() - $A(window).height() > 50
        ? "landscape"
        : "portrait";
    },
    ...
    insert2Input(object, content) {
      ...
      ele.val(
        `${text.substring(0, selectionStart)}${content}${text.substring(
          selectionEnd,
          text.length
        )}`
      );
    ...
    /**
     * 将json字符中的数组的值变成用逗号分割的
     * @param jsonString
     * @returns {string}
     */
    convertJsonArraysToStrings(obj){
    ...
    scrollToView(element, options) {
      ...
      if (
        typeof options.scrollMode !== "undefined" &&
        typeof window.scrollIntoView === "function"
      ) {
  ...
  $.extend({
        ...
        if (obj.hasOwnProperty(prop)) {
          ...
          if (Array.isArray(obj[prop])) {
            ...
            for (let i = 0; i < obj[prop].length; i++) {
              if (
                !Array.isArray(obj[prop][i]) &&
                typeof obj[prop][i] === "object"
              ) {
    ...
    ihttp(options) {
      ...
      $.each(
        $.globalAjaxOptions,
        function (globalOptionName, globalOptionValue) {
      ...
      if (
        ...
        options.url.indexOf("callback=") >= 0
      ) {
      ...
      xhr.onload = function (e) {
        ...
        if (options.statusCode) {
          if (
            $.globalAjaxOptions.statusCode &&
            $.globalAjaxOptions.statusCode[xhr.status]
          )
