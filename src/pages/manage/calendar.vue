  ...
  computed: {
    ...
    list() {
      ...
      if (taskCompleteTemps.length > 0) {
        let tmps = cacheTasks.filter(
          (task) =>
            taskCompleteTemps.includes(task.id) && filterTask(task, false)
        );
        if (tmps.length > 0) {
      ...
      return this.transforTasks(array).map((data) => {
        const isAllday =
          $A.rightExists(data.start_at, "00:00:00") &&
          $A.rightExists(data.end_at, "23:59:59");
        const task = {
          id: data.id,
        ...
        if (data.complete_at) {
          ...
          task.priority += `<span class="overdue">${this.$L(
            "超期未完成"
          )}</span>`;
        }
        if (!task.borderColor) {
  ...
  methods: {
    ...
    setRenderRange() {
      this.$nextTick(() => {
        ...
        this.rangeTime = [
          moment(cal.getDateRangeStart().getTime()).format("YYYY-MM-DD"),
          moment(cal.getDateRangeEnd().getTime()).format("YYYY-MM-DD"),
        ];
      });
    },
          ...
          $A.modalConfirm({
            ...
            onOk: () => {
              return new Promise((resolve, reject) => {
                this.$store
                  .dispatch("removeTask", { task_id: data.id })
                  .then(({ msg }) => {
                    resolve(msg);
                  })
                  .catch(({ msg }) => {
    ...
    onBeforeUpdateSchedule(res) {
      ...
      if (
        changes?.start?.getTime() == schedule?.start?.getTime() &&
        changes?.end?.getTime() == schedule?.end?.getTime()
      ) {
        return;
      }
