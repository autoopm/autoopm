  ...
  <div class="page-file">
    ...
    <div class="file-wrapper" ref="fileWrapper">
      <div class="file-head">
        <div class="file-nav">
          <div
            class="common-nav-back portrait"
            @click="goForward({ name: 'manage-application' }, true)"
          >
            <i class="taskfont">&#xe676;</i>
          </div>
          <h1>{{ $L("文件") }}</h1>
          <div v-if="loadIng == 0" class="file-refresh" @click="getFileList">
            <i class="taskfont">&#xe6ae;</i>
          </div>
        </div>
        <div
          ...
          class="file-status"
          @click="[(uploadShow = true), (packShow = false)]"
        >
          <Loading
            v-if="uploadList.find(({ status }) => status !== 'finished')"
          />
        ...
        <div
          ...
          class="file-status"
          @click="[(packShow = true), (uploadShow = false)]"
        >
          <Loading
            v-if="packList.find(({ status }) => status !== 'finished')"
          />
          <Button v-else shape="circle" icon="md-arrow-round-down"></Button>
        </div>
        <div
          :class="['file-search', searchKey ? 'has-value' : '']"
          @click="onSearchFocus"
        ...
        <div class="file-add">
          <Button
            shape="circle"
            icon="md-add"
            @click.stop="handleRightClick($event, null, true)"
          ></Button>
        </div>
      </div>
      ...
      <div class="file-navigator">
        <ul
          ...
          v-show="showBtnText || (!selectIds.length && !shearFirst)"
        >
          ...
          <li
            ...
            :ref="`nav_${item.id}`"
            @click="browseFolder(item.id)"
          >
            <i v-if="item.share" class="taskfont">&#xe63f;</i>
            <span :title="item.name">{{ item.name }}</span>
            <span v-if="item.share && item.permission == 0" class="readonly">{{
              $L("只读")
            }}</span>
          </li>
        </ul>
        <template v-if="shearFirst">
          <Button
            :disabled="shearFirst.pid == pid"
            size="small"
            type="primary"
            @click="shearTo"
            :style="{ marginLeft: showBtnText ? '12px' : 0 }"
          >
            <div class="file-shear">
              ...
              <span v-show="showBtnText"
                ...
                >"</span
              >
              <span v-if="shearIds.length > 1"
                >{{ $L("等") }}{{ shearIds.length }}{{ $L("个文件") }}</span
              >
            </div>
          </Button>
          <Button type="primary" size="small" @click="clearShear">{{
            $L("取消剪切")
          }}</Button>
        </template>
        <template v-else-if="selectIds.length > 0">
          <Button
            size="small"
            type="info"
            @click="handleContextClick('shearSelect')"
            :style="{ marginLeft: showBtnText ? '12px' : 0 }"
          >
            <Icon type="ios-cut" />
          ...
          <Button
            :disabled="compressedSownloadDisabled"
            size="small"
            type="info"
            @click="downloadZipFile(selectIds)"
          >
        ...
        <div :class="['switch-button', tableMode]">
          <div @click="tableMode = 'block'">
            <i class="taskfont">&#xe60c;</i>
          </div>
          <div @click="tableMode = 'table'">
            <i class="taskfont">&#xe66a;</i>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <template v-else>
          <div
            v-if="fileList.length == 0 && loadIng == 0"
            class="file-no"
            @contextmenu.prevent="handleContextmenu"
          >
            <i class="taskfont">&#xe60b;</i>
              ...
              <li v-for="item in fileList">
                <div
                  ...
                  :class="{
                    ...
                    operate:
                      contextMenuVisible && item.id === contextMenuItem.id,
                  }"
                  :data-id="item.id"
                ...
                >
                  <div
                    class="file-check"
                    :class="{ 'file-checked': selectIds.includes(item.id) }"
                    @click.stop="dropFile(item, 'select')"
                  >
                  ...
                  <div
                    :class="`no-dark-before file-icon ${item.type}${
                      item.share ? ' share' : ''
                    }`"
                  >
                    ...
                    <template v-else-if="isParentShare">
                      <UserAvatarTip
                        :userid="item.created_id"
                        class="share-avatar"
                        :size="20"
                      >
                        <p v-if="item.created_id != item.userid">
                          <strong
                            >{{ $L("成员创建于") }}:
            ...
            <template v-if="contextMenuItem.id">
              ...
              <Dropdown placement="right-start" transfer>
                <DropdownItem divided @click.native.stop="" name="new:">
                  <div class="arrow-forward-item">
                    {{ $L("新建") }}<Icon type="ios-arrow-forward"></Icon>
                  </div>
                </DropdownItem>
                <DropdownMenu slot="list" class="page-file-dropdown-menu">
                  ...
                  >
                    <div
                      :class="`no-dark-before file-item file-icon ${type.value}`"
                    >
                      {{ $L(type.label) }}
                    </div>
                  </DropdownItem>
              ...
              <DropdownItem name="rename" divided>{{
                $L("重命名")
              }}</DropdownItem>
              <DropdownItem
                name="copy"
                :disabled="contextMenuItem.type == 'folder'"
                >{{ $L("复制") }}</DropdownItem
              >
              <DropdownItem
                name="shear"
                :disabled="contextMenuItem.userid != userId"
                >{{ $L("剪切") }}</DropdownItem
              >

              <DropdownItem
                v-if="contextMenuItem.userid == userId"
                name="share"
                divided
                >{{ $L("共享") }}</DropdownItem
              >
              <DropdownItem
                v-else-if="contextMenuItem.share"
                name="outshare"
                divided
                >{{ $L("退出共享") }}</DropdownItem
              >
              ...
              <DropdownItem
                name="link"
                :divided="
                  contextMenuItem.userid != userId && !contextMenuItem.share
                "
                :disabled="contextMenuItem.type == 'folder'"
                >{{ $L("链接") }}</DropdownItem
              >
              <DropdownItem
                ...
                :disabled="
                  contextMenuItem.ext == '' ||
                  (contextMenuItem.userid != userId &&
                    contextMenuItem.permission == 0)
                "
                >{{ $L("下载") }}</DropdownItem
              >
              <DropdownItem
              ...
              <DropdownItem name="delete" divided style="color: red">{{
                $L("删除")
              }}</DropdownItem>
            </template>
            <template v-else>
        ...
        <ul class="content">
          ...
          >
            ...
            <AutoTip
              ...
              "
              class="file-error"
              >{{ item.response.msg }}</AutoTip
            >
            <Progress
              v-else
              :percent="uploadPercentageParse(item.percentage)"
              :stroke-width="5"
            />
            <Icon
              class="file-close"
      ...
      <div class="upload-wrap">
        ...
        <ul class="content">
          <li v-for="(item, index) in packList" :key="index" v-if="index < 100">
            ...
            <Progress
              v-else
              :percent="packPercentageParse(item.percentage)"
              :stroke-width="5"
            />
            <Icon
              class="file-close"
              type="ios-close-circle-outline"
              @click="packList.splice(index, 1)"
            />
          </li>
        </ul>
    ...
    >
      ...
      >
        ...
        <FormItem>
          <Button type="primary" :loading="shareLoad > 0" @click="onShare">{{
            $L("共享")
          }}</Button>
        </FormItem>
      </Form>
    ...
    <!--文件链接-->
    <Modal v-model="linkShow" :title="$L('文件链接')" :mask-closable="false">
      <div>
        ...
        <div class="form-tip" style="padding-top: 6px">
          ...
          >
            <div slot="title">
              <p>
                <strong>{{ $L("注意：刷新将导致原来的链接失效！") }}</strong>
              </p>
            </div>
      ...
      <div slot="footer" class="adaption">
        <Button type="default" @click="linkShow = false">{{
          $L("取消")
        }}</Button>
        <Button type="primary" :loading="linkLoad > 0" @click="linkCopy">{{
          $L("复制")
        }}</Button>
      </div>
    </Modal>
    ...
    <!--预览文件-->
    <PreviewImage v-model="imageShow" :index="imageIndex" :list="imageList" />
    ...
    >
      <ul class="dialog-wrapper-paste" :class="pasteWrapperClass">
        <li v-for="item in pasteItem">
          ...
          <div v-else>
            {{ $L("文件") }}: {{ item.name }} ({{ $A.bytesToSize(item.size) }})
          </div>
        </li>
      </ul>
    </Modal>
...
export default {
            ...
            array.push(
              h(
                "QuickEdit",
                {
                  ...
                  on: {
                    "on-edit-change": (b) => {
                      const file = this.fileLists.find(
                        ({ id }) => id == row.id
                      );
                      if (file) {
                        setTimeout(() => {
            ...
            if (row.share) {
              ...
              iconArray.push(
                h(
                  ...
                  [
                    row.created_id != row.userid
                      ? h("p", [
                          h(
                            "strong",
                            this.$L("成员创建于") + ": " + row.created_at
                          ),
                        ])
                      : h("p", this.$L("所有者创建于") + ": " + row.created_at),
                  ]
                )
              );
            }
      ...
      },
      {
        ...
        render: (h, { row }) => {
          let type = this.types.find(
            ({ value, name }) => value == row.type && name
          );
          if (type) {
  ...
  mounted() {
    this.uploadAccept = this.uploadFormat
      .map((item) => {
        return "." + item;
      })
      .join(",");
  ...
  computed: {
    ...
    shareAlready() {
      let data = this.shareList
        ? this.shareList.map(({ userid }) => userid)
        : [];
    ...
    hasShareFile() {
      ...
      return (
        fileLists.findIndex((file) => file.share && file.userid != userId) !==
        -1
      );
    },
    ...
    navigator() {
      ...
      let array = [];
      while (pid > 0) {
        let file = fileLists.find(
          ({ id, permission }) => id == pid && permission > -1
        );
        if (file) {
          array.unshift(file);
          pid = file.pid;
    ...
    isPreview() {
      return (
        (this.windowPortrait && this.fileInfo.type != "document") ||
        this.fileInfo.permission === 0
      );
    },
  ...
  methods: {
      ...
      this.$nextTick(() => {
        ...
        if (isAddButton === true) {
          this.contextMenuStyles.top = `${
            event.target.clientHeight + event.target.offsetTop - 5
          }px`;
        }
    ...
    browseFolder(id, shakeId = null) {
      if (id > 0) {
        this.goForward({
          name: "manage-file",
          params: { folderId: id, fileId: null, shakeId },
        });
      } else {
        this.searchKey = "";
    ...
    browseFile(id) {
      if (id > 0) {
        this.goForward({
          name: "manage-file",
          params: { folderId: this.pid, fileId: id },
        });
      } else {
    ...
    openFileJudge() {
      ...
      this.$nextTick().then(() => {
        console.log(
          ...
          ">>>nextTick>>>>this.$refs.fileContent"
        );
      });
    },
    ...
    handleContextMenu(row, event) {
      this.handleRightClick(
        event,
        this.fileLists.find(({ id }) => id === row.id) || {}
      );
    },
        ...
        case "download":
          ...
          $A.modalConfirm({
            ...
            onOk: () => {
              this.$store.dispatch(
                "downUrl",
                $A.apiUrl(`file/content?id=${item.id}&down=yes`)
              );
            },
          });
          break;
    ...
    onSendFile() {
      return new Promise((resolve, reject) => {
        ...
        const dialogids = this.sendData
          .filter((value) => $A.leftExists(value, "d:"))
          .map((value) => value.replace("d:", ""));
        const userids = this.sendData.filter(
          (value) => !$A.leftExists(value, "d:")
        );
        this.$store
          .dispatch("call", {
    ...
    shearTo() {
      ...
      if (this.isParentShare) {
        const tmpFile = this.fileLists.find(
          ({ id, share }) => share && this.shearIds.includes(id)
        );
        if (tmpFile) {
          $A.modalError(
            `${tmpFile.name} 当前正在共享，无法移动到另一个共享文件夹内`
          );
          return;
        }
      }
    ...
    packClear() {
      this.packList = this.packList.filter(
    ...
    downloadZipFile(ids) {
      ...
      const allFolder = !ids.some((id) =>
        this.fileLists.some(
          ({ type, id: itemId }) => type !== "folder" && itemId === id
        )
      );
      const typeName = allFolder ? "文件夹" : "文件";
      const fileName =
        ids.length === 1
          ? `【${firstFile.name}】${typeName}`
          : `【${firstFile.name}】等${ids.length}个${typeName}`;
    ...
    onSearchChange() {
      ...
      if (this.searchKey.trim() != "") {
        this.searchTimeout = setTimeout(() => {
          this.loadIng++;
          this.$store
            .dispatch("searchFiles", this.searchKey.trim())
            .then(() => {
              this.loadIng--;
            })
            .catch(() => {
      ...
      this.$store
        ...
        .then(({ data, msg }) => {
          ...
          if (item.permission === -1) {
            let index = this.shareList.findIndex(
    ...
    handleTableSort({ key, order }) {
      $A.IDBSave(
        "cacheFileSort",
        ["asc", "desc"].includes(order) ? { key, order } : {}
      );
    },
    ...
    handleMaxSize(file) {
      ...
      $A.modalWarning({
        ...
        content:
          ...
          file.name +
          " 太大，不能超过：" +
          $A.bytesToSize(this.maxSize * 1024) +
          "。",
      });
    },
