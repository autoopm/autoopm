<template>
  <div
    class="page-manage"
    :class="{ 'show-tabbar': showMobileTabbar, 'not-logged': userId <= 0 }"
  >
    <div
...
<!--      </div>-->
      ...
      <Scrollbar class-name="manage-item" @on-scroll="operateVisible = false">
        <div class="menu-base">
          <ul>
            <li
              @click="toggleRoute('dashboard')"
              :class="classNameRoute('dashboard')"
            >
              <i class="taskfont">&#xe6fb;</i>
              <div class="menu-title">{{ $L("仪表盘") }}</div>
              <Badge
                v-if="dashboardTask.overdue_count > 0"
                class="menu-badge"
                type="error"
                :overflow-count="999"
                :count="dashboardTask.overdue_count"
              />
              <Badge
                v-else-if="dashboardTask.today_count > 0"
                class="menu-badge"
                type="info"
                :overflow-count="999"
                :count="dashboardTask.today_count"
              />
              <Badge
                v-else-if="dashboardTask.all_count > 0"
                class="menu-badge"
                type="primary"
                :overflow-count="999"
                :count="dashboardTask.all_count"
              />
            </li>
<!--            <li-->
<!--              @click="toggleRoute('calendar')"-->
<!--              :class="classNameRoute('calendar')"-->
<!--            >-->
<!--              <i class="taskfont">&#xe6f5;</i>-->
...
<!--                class="menu-badge"-->
<!--                :overflow-count="999"-->
<!--                :text="String(reportUnreadNumber + approveUnreadNumber || '')"-->
<!--              />-->
<!--            </li>-->
          </ul>
          ...
          <ul>
            ...
            >
              <div class="project-h1">
                ...
                <div
                  v-if="item.task_my_num - item.task_my_complete > 0"
                  class="num"
                >
              ...
              <div class="project-h2">
                <p>
                  <em>{{ $L("我的") }}:</em>
                  <span
                    >{{ item.task_my_complete }}/{{ item.task_my_num }}</span
                  >
      ...
      >
        ...
        >
          <div
            :style="{
              userSelect: operateVisible ? 'none' : 'auto',
              height: operateStyles.height,
            }"
          ></div>
          <DropdownMenu slot="list">
      ...
      <ButtonGroup class="manage-box-new-group">
        ...
        <Dropdown @on-click="onAddMenu" trigger="click">
          ...
          <DropdownMenu slot="list">
            <DropdownItem name="task"
            >{{ $L("新建任务") }} ({{ mateName }}+K)</DropdownItem
            >
            <!--            <DropdownItem name="createMeeting"-->
            <!--              >{{ $L("新会议") }} ({{ mateName }}+J)</DropdownItem-->
            <!--            >-->
    ...
    <div class="manage-box-main">
      <keep-alive>
        <router-view
          class="manage-box-view"
          @on-click="onTabbarClick"
        ></router-view>
      </keep-alive>
    </div>

    <!--新建项目-->
    <Modal v-model="addShow" :title="$L('新建项目')" :mask-closable="false">
      <Form
        ref="addProject"
        :model="addData"
        :rules="addRule"
      ...
      <div slot="footer" class="adaption">
        <Button type="default" @click="addShow = false">{{
          $L("取消")
        }}</Button>
        <Button type="primary" :loading="loadIng > 0" @click="onAddProject">{{
          $L("添加")
        }}</Button>
      </div>
    </Modal>
    ...
    <DrawerOverlay v-model="workReportShow" placement="right" :size="1200">
      <Report
        v-if="workReportShow"
        v-model="reportTabs"
        @on-read="$store.dispatch('getReportUnread', 1000)"
      />
    </DrawerOverlay>

    <!--查看所有团队-->
    <DrawerOverlay v-model="allUserShow" placement="right" :size="1380">
      <TeamManagement v-if="allUserShow" />
    </DrawerOverlay>

    <!--查看所有项目-->
    <DrawerOverlay v-model="allProjectShow" placement="right" :size="1200">
      <ProjectManagement v-if="allProjectShow" />
    </DrawerOverlay>

    <!--查看归档项目-->
    <DrawerOverlay v-model="archivedProjectShow" placement="right" :size="1200">
...
export default {
  ...
  mounted() {
    ...
    this.dialogMsgSubscribe = Store.subscribe(
      "dialogMsgPush",
      this.addDialogMsg
    );
    //
  ...
  computed: {
     ...
     */
    msgTodoTotal() {
      let todoNum = this.cacheDialogs.reduce(
        (total, current) => total + (current.todo_num || 0),
        0
      );
      if (todoNum > 0) {
        if (todoNum > 99) {
    ...
    menu() {
      ...
      const array = [{ path: "taskBrowse", name: "最近打开的任务" }];
      if (userIsAdmin) {
        array.push(
          ...[
    ...
    showMobileTabbar() {
      if (
        this.routeName === "manage-project" &&
        !/^\d+$/.test(this.$route.params.projectId)
      ) {
        return true;
      }
      return [
    ...
    "cacheProjects.length": {
      handler() {
        this.$nextTick((_) => {
          ...
          const lastEl = $A.last(
            $A.getObject(menuProject, "children.0.children")
          );
          if (lastEl) {
  ...
  methods: {
    ...
    classNameProject(item) {
      return {
        active:
          this.routeName === "manage-project" &&
          this.$route.params.projectId == item.id,
    ...
    selectChange(index) {
      this.$nextTick(() => {
        this.$set(
          this.addData,
          "columns",
          this.columns[index].columns.join(",")
        );
      });
    },
    ...
    addDialogMsg(data) {
      ...
      if (
        this.windowActive &&
        data.dialog_id === $A.last(this.dialogIns)?.dialog_id
      ) {
      ...
      switch (type) {
        case "text":
          body = $A.getMsgTextPreview(
            msg.type === "md" ? MarkdownPreview(msg.text) : msg.text
          );
          break;
        case "file":
          body =
            "[" + this.$L(msg.type == "img" ? "图片信息" : "文件信息") + "]";
          break;
        default:
          return;
      }
        ...
        if (this.__notificationId === id) {
          ...
          if (this.$isEEUiApp) {
            ...
            this.$Electron.sendMessage("openNotification", {
              icon: $A.originUrl("images/logo2.png"),
              title,
              body,
              data,
              tag: "dialog",
      ...
      if (dialog) {
        ...
        this.$store
          ...
          .then(({ data }) => notificationFuncA(data.name))
          .catch(() => {});
      }
    },
    ...
    handleLongpress(event, el) {
      ...
      const projectItem = this.projectLists.find(
