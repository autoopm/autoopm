...
export default {
  ...
  watch: {
    ...
    userId: {
      handler() {
        ...
        if (this.userId > 0) {
          this.wuKongIMConnection();  //wukongim连接
           // 初始化WukongIM
          if (this.$isEEUiApp) {
          ...
          $A.IDBString("logOpen").then((r) => {
            $A.openLog = r === "open";
            if ($A.openLog) {
              $A.loadScript("js/vconsole.min.js")
                .then((_) => {
  ...
  methods: {
    ...
    wuKongIMConnection(){
      ...
      WKSDK.shared().config.addr = 'ws://82.157.62.190:5200'; // 默认端口为5200 //TODO 这里还要改成当前的服务器ip
// 认证信息
    ...
    synchThemeLanguage() {
      if (this.isSoftware) {
        this.iframes = this.iframes.filter(
          ({ key }) => key != "synchThemeLanguage"
        );
        this.iframes.push({
          key: "synchThemeLanguage",
          url: $A.apiUrl(
            `../setting/theme_language?theme=${
