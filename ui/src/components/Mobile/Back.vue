...
export default {
  ...
  methods: {
    ...
    touchmove(event) {
      ...
      const pageX =
        event.type === "touchmove" ? event.targetTouches[0].pageX : event.pageX;
      const pageY =
        event.type === "touchmove" ? event.targetTouches[0].pageY : event.pageY;
      if (typeof this.isScrolling === "undefined") {
        this.isScrolling = !!(
          this.isScrolling ||
          Math.abs(pageY - this.touchesStart.y) >
            Math.abs(pageX - this.touchesStart.x)
        );
      }
      if (this.isScrolling) {
      ...
      if (this.routeName === "manage-file") {
        if (this.fileFolderId > 0) {
          const file = this.fileLists.find(
            ({ id, permission }) => id == this.fileFolderId && permission > -1
          );
          if (file) {
            const prevFile = this.fileLists.find(
              ({ id, permission }) => id == file.pid && permission > -1
            );
            if (prevFile) {
              this.goForward({
                name: "manage-file",
                params: { folderId: prevFile.id, fileId: null },
              });
              return;
            }
          }
