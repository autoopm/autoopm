      ...
      <div v-else class="edit-header">
        <div class="header-title">
          <EPopover
            v-if="!equalContent"
            v-model="unsaveTip"
            class="file-unsave-tip"
          >
            <div class="task-detail-delete-file-popover">
              <p>{{ $L("未保存当前修改内容？") }}</p>
              <div class="buttons">
                <Button size="small" type="text" @click="unSaveGive">{{
                  $L("放弃")
                }}</Button>
                <Button size="small" type="primary" @click="onSaveSave">{{
                  $L("保存")
                }}</Button>
              </div>
            </div>
        ...
        <div class="header-user">
          <ul>
            <li
              v-for="(userid, index) in editUser"
              :key="index"
              v-if="index <= 10"
            >
            ...
            <li
              v-if="editUser.length > 10"
              class="more"
              :title="editUser.length"
            >
        ...
        <div
          v-if="file.type == 'document' && contentDetail && !windowPortrait"
          class="header-hint"
        >
          <ButtonGroup size="small" shape="circle">
            <Button
              :type="`${contentDetail.type == 'md' ? 'primary' : 'default'}`"
              @click="setTextType('md')"
              >{{ $L("MD编辑器") }}</Button
            >
            <Button
              :type="`${contentDetail.type != 'md' ? 'primary' : 'default'}`"
              @click="setTextType('text')"
              >{{ $L("文本编辑器") }}</Button
            >
          </ButtonGroup>
        </div>
        ...
        >
          <a href="javascript:void(0)"
            >{{ $L("导出") }}<Icon type="ios-arrow-down"></Icon
          ></a>
        ...
        <template v-if="!file.only_view">
          <div class="header-icons">
            <ETooltip
              ...
              :content="$L('文件链接')"
            >
              <div class="header-icon" @click="handleClick('link')">
                <i class="taskfont">&#xe785;</i>
              </div>
            </ETooltip>
            <EPopover v-model="historyShow" trigger="click">
              ...
              <ETooltip
                slot="reference"
                ref="historyTip"
                :disabled="$isEEUiApp || windowTouch || historyShow"
                :content="$L('历史版本')"
              >
      ...
      <div class="content-body">
        ...
        <template v-if="file.type == 'document'">
          ...
          <TEditor
            ...
            v-model="contentDetail.content"
            height="100%"
            @editorSave="handleClick('saveBefore')"
          />
        </template>
        <Drawio
          v-else-if="file.type == 'drawio'"
          ref="myFlow"
        ...
        />
        <Minder
          v-else-if="file.type == 'mind'"
          ref="myMind"
          v-model="contentDetail"
          @saveData="handleClick('saveBefore')"
        />
        <AceEditor
          v-else-if="['code', 'txt'].includes(file.type)"
          v-model="contentDetail.content"
          :ext="file.ext"
          @saveData="handleClick('saveBefore')"
        />
        <OnlyOffice
          v-else-if="['word', 'excel', 'ppt'].includes(file.type)"
          v-model="contentDetail"
          :documentKey="documentKey"
          @on-document-ready="handleClick('officeReady')"
        />
      </div>
    </template>
...
import MDPreview from "../../../components/MDEditor/preview";
              ...
              if (this.value && data.id == this.fileId) {
                $A.modalConfirm({
                  ...
                  content:
                    ...
                    info.nickname +
                    "）更新了内容，<br/>更新时间：" +
                    $A.formatDate("Y-m-d H:i:s", info.time) +
                    "。<br/><br/>点击【确定】加载最新内容。",
                  onOk: () => {
                    this.getContent();
                  },
