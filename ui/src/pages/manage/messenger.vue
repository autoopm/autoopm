...
<!--                                                <span>{{$A.getMsgSimpleDesc(dialog.last_msg)}}</span>-->
            ...
            <div v-if="activeNum > 0 && routeName === 'manage-messenger'" class="messenger-msg">
                ...
                <DialogWrapper v-if="windowLandscape && dialogId !== ''" :dialogId="dialogId" @on-active="scrollIntoActive" :auto-focus="$A.isDesktop()" is-messenger/>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
      ...
      listTouch() {
        ...
        filterDialog(dialog) {
            if ((dialog.id !== '' && dialog.id == this.dialogId) || dialog.top_at || dialog.todo_num > 0 || $A.getDialogNum(dialog) > 0) {
                return true
            }
