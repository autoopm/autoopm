...
(function (window) {
  ...
  $.extend({
    ...
    apiUrl(str) {
      if (str == "privacy") {
        ...
        if (apiHome == "" || apiHome == "public") {
          return "https://www.codeetool.com/privacy.html";
        }
        str = "../privacy.html";
      }
      if (
      ...
      return str
      /**
       * else if (str.indexOf("/apps/okr") === -1) {
        str = window.location.origin + "/request/api/" + str;
      }
       */
      // if (typeof window.systemInfo.apiUrl === "string") {
      //   str = window.systemInfo.apiUrl + str;
      ...
      //       : "http://192.168.31.130:8200";
      //   const proxy = env === "development" ? "/request/api/" : "/api/";
      //   str = host + proxy + str;
      //   // str = "http://192.168.100.134:8081/api/";
      //   console.log("run to here and--> "+str)
      // }
      // while (str.indexOf("/../") !== -1) {
      //   str = str.replace(/\/(((?!\/).)*)\/\.\.\//, "/");
      // }
    ...
    formatSeconds(second) {
      ...
      if (days > 0) {
        if (hours > 0) duration = days + "d," + this.formatBit(hours) + "h";
          ...
          duration = days + "d," + this.formatBit(minutes) + "min";
        else if (seconds > 0)
          duration = days + "d," + this.formatBit(seconds) + "s";
        else duration = days + "d";
      } else if (hours > 0)
        duration =
          this.formatBit(hours) +
          ":" +
          this.formatBit(minutes) +
          ":" +
          this.formatBit(seconds);
      else if (minutes > 0)
        duration = this.formatBit(minutes) + ":" + this.formatBit(seconds);
    ...
    dateRangeUtil: {
      ...
      getPreviousSeason() {
        ...
        let currentYear = currentDate.getFullYear();
        //上季度的第一天
        let priorSeasonFirstDay = this.getPriorSeasonFirstDay(
          currentYear,
          currentMonth
        );
        //上季度的最后一天
     ...
     */
    timeOptionShortcuts() {
      const startSecond = $A.Date(
        $A.formatDate("Y-m-d 00:00:00", Math.round(new Date().getTime() / 1000))
      );
      const lastSecond = (e) => {
      ...
      return [
        ...
        },
        {
          ...
          value() {
            return [
              startSecond,
              lastSecond($A.getSpecifyDate("本周结束", true).getTime()),
            ];
          },
        },
        {
          ...
          value() {
            return [
              startSecond,
              lastSecond($A.getSpecifyDate("本月结束", true).getTime()),
            ];
          },
        },
        {
    ...
    dialogTags(dialog) {
      ...
      if (dialog.type == "group") {
        if (
          ["project", "task"].includes(dialog.group_type) &&
          $A.isJson(dialog.group_info)
        ) {
    ...
    getMsgTextPreview(text, imgClassName = null) {
      ...
      //
      text = text.replace(
        /<img\s+class="emoticon"[^>]*?alt="(\S+)"[^>]*?>/g,
        "[$1]"
      );
      text = text.replace(
        /<img\s+class="emoticon"[^>]*?>/g,
        `[${$A.L("动画表情")}]`
      );
      if (imgClassName) {
        text = text.replace(
          /<img\s+class="browse"[^>]*?src="(\S+)"[^>]*?>/g,
          function (res, src) {
    ...
    formatTextMsg(text, userid) {
      ...
      const atReg = new RegExp(
        `<span class="mention user" data-id="${userid}">`,
        "g"
      );
      ...
      text = text.replace(
        atReg,
        `<span class="mention me" data-id="${userid}">`
      );
      // 处理内容连接
      if (/https*:\/\//.test(text)) {
        text = text
        ...
        array.some((res) => {
          ...
          if (widthMatch && heightMatch) {
            ...
            const value = res
              ...
              .replace(
                heightReg,
                `original-height="${height}" style="width:${scale.width}px;height:${scale.height}px"`
              );
            text = text.replace(res, value);
          } else {
            text = text.replace(
              res,
              `<div class="no-size-image-box">${res}</div>`
            );
          }
        });
      }
    ...
    getMsgSimpleDesc(data, imgClassName = null) {
      if ($A.isJson(data)) {
        switch (data.type) {
          case "text":
            return $A.getMsgTextPreview(
              data.msg.type === "md"
                ? MarkdownPreview(data.msg.text)
                : data.msg.text,
              imgClassName
            );
          case "word-chain":
            return (
              `[${$A.L("接龙")}]` +
              $A.getMsgTextPreview(data.msg.text, imgClassName)
            );
          case "vote":
            return (
              `[${$A.L("投票")}]` +
          ...
          case "tag":
            return `[${$A.L(
              data.msg.action === "remove" ? "取消标注" : "标注"
            )}] ${$A.getMsgSimpleDesc(data.msg.data)}`;
          case "top":
            return `[${$A.L(
              data.msg.action === "remove" ? "取消置顶" : "置顶"
            )}] ${$A.getMsgSimpleDesc(data.msg.data)}`;
          case "todo":
            return `[${$A.L(
              data.msg.action === "remove"
                ? "取消待办"
                : data.msg.action === "done"
                ? "完成"
                : "设待办"
            )}] ${$A.getMsgSimpleDesc(data.msg.data)}`;
          case "notice":
  ...
  $.extend({
    ...
    modalConfig(config) {
      ...
      config.title =
        config.title ||
        (typeof config.render === "undefined" ? "温馨提示" : "");
          ...
          return h("div", [
            ...
            h("Input", {
              ...
              on: {
                ...
                "on-enter": (e) => {
                  $A(e.target)
                    .parents(".ivu-modal-body")
                    .find(".ivu-btn-primary")
                    .click();
                },
              },
            }),
    ...
    noticeConfig(config) {
      ...
      config.title = $A.L(
        config.title || (typeof config.render === "undefined" ? "温馨提示" : "")
      );
  ...
  $.extend({
    dark: {
      utils: {
        ...
        reverseFilter() {
          return (
            "-webkit-filter: invert(100%) hue-rotate(180deg) contrast(110%) !important; " +
