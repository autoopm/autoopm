...
export default {
  ...
  init({ state, dispatch }) {
    return new Promise(async (resolve) => {
      ...
      state.cacheDialogs = (await $A.IDBArray("cacheDialogs")).map((item) =>
        Object.assign(item, {
          loading: false,
  ...
  call({ state, dispatch }, params) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (/^https*:\/\/public\//.test(params.url)) {
        reject({ code: 400, data: {}, msg: "No server address" });
        return;
      }
      // 加密传输
      const encrypt = [];
      ...
      //       encrypt.push(
      //         `encrypt_type=${state.apiKeyData.type};encrypt_id=${state.apiKeyData.id}`
      //       );
      //       params.method = "post"; // 加密传输时强制使用post
      //       params.data = {
      //         encrypted: await dispatch("pgpEncryptApi", params.data),
      //       };
      //     }
      //   }
      //   encrypt.push(
      //     "client_type=pgp;client_key=" +
      //       (await dispatch("pgpGetLocalKey")).publicKeyB64
      //   );
      // }
      // if (encrypt.length > 0) {
      ...
      if (
        params.spinner === true ||
        (typeof params.spinner === "number" && params.spinner > 0)
      ) {
      ...
      params.success = async (result, status, xhr) => {
        ...
        if (code === -2 && params.checkNick !== false) {
          // 需要昵称
          dispatch("userEditInput", "nickname")
            .then(() => {
              dispatch("call", cloneParams).then(resolve).catch(reject);
            })
            .catch((err) => {
              reject({ ret: -1, data, message: err || $A.L("请设置昵称！") });
            });
          return;
        }
        if (code === -3 && params.checkTel !== false) {
          // 需要联系电话
          dispatch("userEditInput", "tel")
            .then(() => {
              dispatch("call", cloneParams).then(resolve).catch(reject);
            })
            .catch((err) => {
              reject({ code: -1, data, message: err || $A.L("请设置联系电话！") });
            });
          return;
        }
        if (code === 200) {
          ...
          //TODO 更改了这里
          reject({ code, data, msg: message || "Unknown error" });
          //
          if (code === -4001) {
      ...
      params.error = (xhr, status) => {
        ...
        if (networkException) {
          reject({ code: 400, data: {}, msg: "Network exception" });
        } else {
          reject({ code:400, data: {}, msg: "System error" });
        }
      ...
      if (params.websocket === true) {
        ...
        const apiTimeout = setTimeout(() => {
          const WListener = state.ajaxWsListener.find(
            (item) => item.apiWebsocket == apiWebsocket
          );
          if (WListener) {
            WListener.complete();
          ...
          state.ajaxWsListener = state.ajaxWsListener.filter(
            (item) => item.apiWebsocket != apiWebsocket
          );
  ...
  toggleProjectParameter({ state }, data) {
    ...
    if (project_id) {
      let index = state.cacheProjectParameter.findIndex(
        (item) => item.project_id == project_id
      );
      if (index === -1) {
        state.cacheProjectParameter.push(
          $A.projectParameterTemplate(project_id)
        );
        index = state.cacheProjectParameter.findIndex(
          (item) => item.project_id == project_id
        );
      }
  ...
  getBasicData({ state, dispatch }, timeout) {
    ...
    if (typeof timeout === "number") {
      ...
      if (timeout > -1) {
        window.__getBasicDataTimer = setTimeout(
          (_) => dispatch("getBasicData", null),
          timeout
        );
      }
      return;
  ...
  saveUserInfoBase({ state, dispatch }, info) {
    return new Promise(async (resolve) => {
      ...
      userInfo.token =
        userInfo.userid > 0 ? userInfo.token || state.userToken : "";
      ...
      $A.eeuiAppSendMessage({
        ...
        token: state.userToken,
        dirUrl:
          $A.apiUrl("../api/file/content/upload") + `?token=${state.userToken}`,
        chatUrl:
          $A.apiUrl("../api/dialog/msg/sendfiles") +
          `?token=${state.userToken}`,
      });
      //
      resolve();
  ...
  saveUserInfo({ state, dispatch }, info) {
    return new Promise(async (resolve) => {
      ...
      if (state.userId > 0) {
        state.cacheUserBasic = state.cacheUserBasic.filter(
  ...
  getUserBasic({ state, dispatch }, data) {
    ...
    let timeout = 0;
    list.some((item) => {
      let temp = state.cacheUserBasic.find(
    ...
    //需要判断是对象还是数据
    var userids = [...new Set(array.map(({ userid }) => userid))];
    var useridParam = userids.join(',')

    dispatch("call", {
      url: "users/basic",
      data: {
        // userids: [...new Set(array.map(({ userid }) => userid))],
        userids:useridParam  //TODO 临时写死,其他情况到时候再一起改
        // userid:userid.join(",")
      },
      skipAuthError: true,
    })
      .then((result) => {
        ...
        array.forEach((value) => {
          let data =
            result.data.find(({ userid }) => userid == value.userid) ||
  ...
  saveUserBasic({ state }, data) {
    ...
    //
    const index = state.cacheUserBasic.findIndex(
  ...
  logout({ state, dispatch }, appendFrom = true) {
    dispatch("handleClearCache", {}).then(() => {
      let from = ["/", "/login"].includes(window.location.pathname)
        ? ""
  ...
  handleClearCache({ state, dispatch }, userData) {
    return new Promise(async (resolve) => {
      try {
        ...
        const cacheProjectParameter = await $A.IDBArray(
        ...
        dispatch(
          "saveUserInfoBase",
          $A.isJson(userData) ? userData : userInfo
        ).then(resolve);
      } catch (e) {
        resolve();
      }
  ...
  saveFile({ state, dispatch }, data) {
    ...
    if ($A.isArray(data)) {
      ...
      if (index > -1) {
        state.fileLists.splice(
          index,
          1,
          Object.assign(base, state.fileLists[index], data)
        );
      } else {
  ...
  packProgress({ state, dispatch }, data) {
    ...
    //
    const index = state.filePackLists.findIndex(
      ({ name }) => name == data.name
    );
    if (index > -1) {
    ...
    return new Promise(function (resolve, reject) {
      dispatch("call", {
        ...
        .then((result) => {
          ...
          state.fileLists = state.fileLists.filter(
            (item) => item.pid != pid || ids.includes(item.id)
          );
          $A.IDBSave("fileLists", state.fileLists, 600);
          //
      ...
      state.cacheDialogs.some((dialog) => {
        if (
          ...
          for (let key in dialog.group_info) {
            if (
              !dialog.group_info.hasOwnProperty(key) ||
              data[key] === undefined
            )
              continue;
    ...
    ids.some((id) => {
      ...
      if (index > -1) {
        dispatch(
          "forgetTask",
          state.cacheTasks
            .filter((item) => item.projectId === project_id)
            .map((item) => item.id)
        );
        dispatch(
          "forgetColumn",
          state.cacheColumns
            .filter((item) => item.projectId === project_id)  //这里也需要更改
            .map((item) => item.id)
        );
  ...
  saveColumn({ state, dispatch }, data) {
    ...
    if ($A.isArray(data)) {
      ...
      if (index > -1) {
        state.cacheColumns.splice(
          index,
          1,
          Object.assign({}, state.cacheColumns[index], data)
        );
      } else {
  ...
  forgetColumn({ state, dispatch }, column_id) {
    ...
    Array.from(new Set(project_ids)).some((id) =>
      dispatch("getProjectOne", id).catch(() => {})
    );
    //
    $A.IDBSave("cacheColumns", state.cacheColumns);
  },
    ...
    return new Promise(function (resolve, reject) {
      ...
      dispatch("call", {
        ...
        .then(({ data }) => {
          ...
          state.cacheColumns = state.cacheColumns.filter(
            (item) => item.projectId != project_id || ids.includes(item.id)
          );
          //
          ...
          // 判断只有1列的时候默认版面为表格模式
          if (
            state.cacheColumns.filter((item) => item.projectId == project_id)
              .length === 1
          ) {
            const cache =
              state.cacheProjectParameter.find(
                (item) => item.projectId == project_id
              ) || {};
            if (
              typeof cache.menuInit === "undefined" ||
              cache.menuInit === false
            ) {
  ...
  saveTask({ state, dispatch }, data) {
    ...
    if ($A.isArray(data)) {
      ...
      // console.log('task------------------>333333')
    } else if ($A.isJson(data)) {
      data._time = $A.Time();
      if (data.flow_item_name && data.flow_item_name.indexOf("|") !== -1) {
        [data.flow_item_status, data.flow_item_name] =
          data.flow_item_name.split("|");
      }
      //
      if (typeof data.archived_at !== "undefined") {
        state.cacheTasks
          .filter((task) => task.parent_id == data.id)
          .some((task) => {
            dispatch(
              "saveTask",
              Object.assign(task, {
      ...
      if (typeof data.update_marking !== "undefined") {
        updateMarking = $A.isJson(data.update_marking)
          ? data.update_marking
          : {};
      ...
      if (index > -1) {
        state.cacheTasks.splice(
          index,
          1,
          Object.assign({}, state.cacheTasks[index], data)
        );
      } else {
      ...
      if (
        ...
        (data.parent_id > 0 &&
          state.cacheTasks.findIndex(({ id }) => id == data.parent_id) === -1)
      ) {
  ...
  forgetTask({ state, dispatch }, task_id) {
    ...
    //
    const ids = ($A.isArray(task_id) ? task_id : [task_id]).filter(
      (id) => id != state.taskArchiveView
    );
    const parent_ids = [];
    const project_ids = [];
    ids.some((id) => {
      ...
      state.cacheTasks
        ...
        .some((childTask) => {
          let cIndex = state.cacheTasks.findIndex(
            (task) => task.id == childTask.id
          );
          if (cIndex > -1) {
    ...
    Array.from(new Set(parent_ids)).some((id) =>
      dispatch("getTaskOne", id).catch(() => {})
    );
    Array.from(new Set(project_ids)).some((id) =>
      dispatch("getProjectOne", id).catch(() => {})
    );
  ...
  increaseTaskMsgNum({ state }, data) {
    ...
    //
    if (data.dialog_id) {
      const task = state.cacheTasks.find(
        ({ dialog_id }) => dialog_id === data.dialog_id
      );
    ...
    if (typeof timeout === "number") {
      if (timeout > -1) {
        window.__getTaskForDashboard = setTimeout(
          (_) => dispatch("getTaskForDashboard", null),
          timeout
        );
      }
      return;
  ...
  saveTaskContent({ state, dispatch }, data) {
    ...
    if ($A.isArray(data)) {
      ...
      const index = state.taskContents.findIndex(
        ({ task_id }) => task_id == data.task_id
      );
      if (index > -1) {
        state.taskContents.splice(
          index,
          1,
          Object.assign({}, state.taskContents[index], data)
        );
      } else {
  ...
  taskUpdate({ state, dispatch }, data) {
    ...
    //   console.log(key);
    //   if (key === "times") {
    //     var timesString = data[key].join(",");
    //     data[key] = timesString;
    //
    //   }
    // }
  ...
  taskBeforeUpdate({ state, dispatch }, data) {
    return new Promise(function (resolve, reject) {
      ...
      if (typeof post.times !== "undefined") {
        ...
        const currentTask = state.cacheTasks.find(
          ({ id }) => id == post.task_id
        );
          ...
          if (currentTask.parent_id > 0) {
            ...
            if (post.times[0]) {
              state.cacheTasks.some((parentTask) => {
                ...
                if (n1 < o1) {
                  content =
                    "新设置的子任务开始时间在主任务时间之外，修改后将同步修改主任务"; // 子任务开始时间 < 主任务开始时间
                  return true;
                }
                if (n2 > o2) {
                  content =
                    "新设置的子任务结束时间在主任务时间之外，修改后将同步修改主任务"; // 子任务结束时间 > 主任务结束时间
                  return true;
                }
              });
            }
      ...
      dispatch("call", {
        ...
        .then((result) => {
          ...
          data.turns.some((item) => {
            const index = state.taskFlowItems.findIndex(
              ({ id }) => id == item.id
            );
            if (index > -1) {
            ...
            if (
              ...
              state.cacheTasks
                .filter(({ flow_item_id }) => flow_item_id == item.id)
                .some((task) => {
                  dispatch("saveTask", {
                    id: task.id,
          ...
          //
          delete data.turns;
          const index = state.taskFlows.findIndex(
            ({ task_id }) => task_id == data.task_id
          );
          if (index > -1) {
  ...
  forgetTaskCompleteTemp({ state }, task_id) {
    if (task_id === true) {
      ...
      state.taskCompleteTemps = state.taskCompleteTemps.filter(
        (id) => id != task_id
      );
    }
  },
  ...
  taskDefaultStartTime({ state }, value) {
    return new Promise((resolve) => {
      if (/(\s|^)([0-2]\d):([0-5]\d)(:\d{1,2})*$/.test(value)) {
        value = value.replace(
          /(\s|^)([0-2]\d):([0-5]\d)(:\d{1,2})*$/,
          "$1" + state.systemConfig.task_default_time[0]
        );
      }
      resolve(value);
    });
  ...
  taskDefaultEndTime({ state }, value) {
    return new Promise((resolve) => {
      if (/(\s|^)([0-2]\d):([0-5]\d)(:\d{1,2})*$/.test(value)) {
        value = value.replace(
          /(\s|^)([0-2]\d):([0-5]\d)(:\d{1,2})*$/,
          "$1" + state.systemConfig.task_default_time[1]
        );
      }
      resolve(value);
    });
  ...
  updateDialogLastMsg({ state, dispatch }, data) {
    ...
    if ($A.isArray(data)) {
      ...
      const index = state.cacheDialogs.findIndex(
        ({ id }) => id == data.dialog_id
      );
      if (index > -1) {
        const updateData = {
          id: data.dialog_id,
  ...
  getDialogUnreads({ state, dispatch }) {
    return new Promise(async (resolve) => {
      ...
      //
      const dialog = $A
        .cloneJSON(state.cacheDialogs)
        .filter(({ last_at }) => last_at)
        .sort((a, b) => {
      ...
      .then(({ data }) => {
        if ($A.arrayLength(data) > 0) {
          if (dialog_id > 0) {
            ...
            state.dialogTodos = state.dialogTodos.filter(
              (item) => item.dialog_id != dialog_id
            );
          }
    ...
    ids.some((id) => {
      ...
      if (index > -1) {
        dispatch(
          "forgetDialogMsg",
          state.dialogMsgs
            .filter((item) => item.dialog_id == dialog_id)
            .map((item) => item.id)
        );
  ...
  saveInDialog({ state, dispatch }, data) {
    ...
    if (index > -1) {
      state.dialogIns.splice(
        index,
        1,
        Object.assign({}, state.dialogIns[index], data)
      );
    } else {
    ...
    if (
      state.dialogMsgs.length > msg_max &&
      state.dialogHistory.length > retain_num
    ) {
      ...
      historys.forEach((id) => {
        if (
          newIds.length < retain_num ||
          state.dialogIns.findIndex((item) => item.dialog_id == id) > -1
        ) {
          newIds.push(id);
        } else {
          delIds.push(id);
      ...
      if (delIds.length > 0) {
        state.dialogMsgs = state.dialogMsgs.filter(
          (item) => !delIds.includes(item.dialog_id)
        );
  ...
  saveDialogTodo({ state, dispatch }, data) {
    ...
    if ($A.isArray(data)) {
      ...
      if (index > -1) {
        state.dialogTodos.splice(
          index,
          1,
          Object.assign({}, state.dialogTodos[index], data)
        );
      } else {
  ...
  saveDialogMsgTop({ state, dispatch }, data) {
    ...
    if ($A.isArray(data)) {
      ...
      state.dialogMsgTops = state.dialogMsgTops.filter(
        (item) => item.dialog_id != data.dialog_id
      );
      const index = state.dialogMsgTops.findIndex((item) => item.id == data.id);
      if (index > -1) {
        state.dialogMsgTops.splice(
          index,
          1,
          Object.assign({}, state.dialogMsgTops[index], data)
        );
      } else {
  ...
  forgetDialogMsgTopForMsgId({ state, dispatch }, msg_id) {
    ...
    //
    const index = state.dialogMsgTops.findIndex(
  ...
  saveDialogDraft({ state, dispatch }, data) {
    state.dialogDraftTimer[data.id] &&
      clearInterval(state.dialogDraftTimer[data.id]);
    state.dialogDraftTimer[data.id] = setTimeout(
      (_) => {
  ...
  saveDialogMsg({ state, dispatch }, data) {
    ...
    if ($A.isArray(data)) {
      ...
      $A.IDBSave("dialogMsgs", state.dialogMsgs, 600);
      //
      const dialog = state.cacheDialogs.find(
        ({ id, last_msg }) =>
          id == data.dialog_id && last_msg && last_msg.id === data.id
      );
      if (dialog) {
        dispatch("saveDialog", {
          id: data.dialog_id,
  ...
  getDialogMsgs({ state, dispatch, getters }, data) {
    return new Promise((resolve, reject) => {
      ...
      if (typeof data.save_before !== "undefined") {
        saveBefore =
          typeof data.save_before === "function" ? data.save_before : (_) => {};
        delete data.save_before;
      }
      if (typeof data.save_after !== "undefined") {
        saveAfter =
          typeof data.save_after === "function" ? data.save_after : (_) => {};
        delete data.save_after;
      }
      if (typeof data.clear_before !== "undefined") {
        clearBefore =
          typeof data.clear_before === "boolean" ? data.clear_before : false;
      ...
      dispatch("call", {
        ...
        .then((result) => {
          ...
          if ($A.isJson(resData.dialog)) {
            ...
            state.dialogMsgs = state.dialogMsgs.filter((item) => {
              return (
                item.dialog_id != data.dialog_id ||
                ids.includes(item.id) ||
                $A.Time(item.created_at) >= resData.time
              );
            });
            $A.IDBSave("dialogMsgs", state.dialogMsgs, 600);
        ...
        .catch((e) => {
          resolve(""); //TODO 模拟返回
          console.error("进到11111111111111111111111111这里异常............................")
          console.warn(e);
          reject(e);
        })
        .finally((_) => {
          // 将原数据清除，避免死循环
          if (data.prev_id) {
            const prevMsg = state.dialogMsgs.find(
              ({ prev_id }) => prev_id == data.prev_id
            );
            if (prevMsg) {
              prevMsg.prev_id = 0;
            }
          ...
          if (data.next_id) {
            const nextMsg = state.dialogMsgs.find(
    ...
    return new Promise(function (resolve, reject) {
      ...
      dispatch("call", {
        ...
        .then(({ data }) => {
          ...
          const lastNotExist =
            state.dialogMsgs.findIndex(({ id }) => id == lastId) === -1;
          ...
          if (data.next_page_url && data.current_page < 5 && lastNotExist) {
            requestData.page++;
            dispatch("getDialogLatestMsgs", requestData)
              .then(resolve)
              .catch(reject);
          } else {
            resolve();
  ...
  dialogMsgRead({ state, dispatch }, data) {
    if ($A.isJson(data)) {
      ...
      if (dialog) {
        ...
        if (mark) {
          state.readEndMark[data.dialog_id] = Math.max(
            data.id,
            $A.runNum(state.readEndMark[data.dialog_id])
          );
          dispatch("saveDialog", dialog);
        }
      }
  ...
  streamDialogMsg({ state, dispatch }, streamUrl) {
    ...
    sse.subscribe(["append", "replace", "done"], (type, e) => {
      switch (type) {
        ...
        case "done":
          const index = state.dialogSseList.findIndex(
            (item) => sse === item.sse
          );
          if (index > -1) {
  ...
  showSpinner({ state }, delay) {
    ...
    state.floatSpinnerTimer.push({
      ...
      timer: setTimeout(
        (_) => {
          state.floatSpinnerTimer = state.floatSpinnerTimer.filter(
            (item) => item.id !== id
          );
  ...
  websocketConnection({ state, dispatch }) {
    ...
    //
    //     case "receipt":
    //       typeof state.wsCall[msgId] === "function" &&
    //         state.wsCall[msgId](msgDetail.body, true);
    ...
    //     default:
    //       msgId &&
    //         dispatch("websocketSend", { type: "receipt", msgId }).catch(
    //           (_) => {}
    //         );
    ...
    //                   if (
    //                     count > 5 ||
    //                     state.dialogMsgs.find(({ id }) => id == data.id)
    //                   ) {
    //                     // 更新消息列表
  ...
  websocketPath({ state, dispatch }, path) {
    ...
    state.wsPathTimeout = setTimeout(() => {
      if (state.wsPathValue == path) {
        dispatch("websocketSend", { type: "path", data: { path } }).catch(
          (_) => {}
        );
      }
    }, 1000);
  ...
  pgpGenerate({ state }) {
    return new Promise(async (resolve) => {
      ...
      data.publicKeyB64 = $urlSafe(
        data.publicKey.replace(
          /\s*-----(BEGIN|END) PGP PUBLIC KEY BLOCK-----\s*/g,
          ""
        )
      );
      resolve(data);
  ...
  pgpEncrypt({ state, dispatch }, data) {
    return new Promise(async (resolve) => {
      ...
      const publicKeyArmored =
        data.publicKey ||
        data.key ||
        (await dispatch("pgpGetLocalKey")).publicKey;
  ...
  pgpDecrypt({ state, dispatch }, data) {
    return new Promise(async (resolve) => {
      ...
      const privateKeyArmored =
        data.privateKey ||
        data.key ||
        (await dispatch("pgpGetLocalKey")).privateKey;
  ...
  showMeetingWindow(
    { state },
    { type, meetingid, meetingdisabled, meetingSharekey }
  ) {
    state.meetingWindow = {
      show: true,
      type: type,
